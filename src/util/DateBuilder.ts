...
export default class DateBuilder {
    ...
    private static instance? : DateBuilder = undefined;

    static getInstance() : DateBuilder {
        if (DateBuilder.instance == undefined) {
            DateBuilder.instance = new DateBuilder();
        } 
        return DateBuilder.instance as DateBuilder;
    }
    ...
    private constructor() {
        ...
        const today = $.date.getMoment(DateType.TODAY)
        const tomorrow = $.date.getMoment(DateType.TOMORROW)
        const yesterday = $.date.getMoment(DateType.YESTERDAY)
